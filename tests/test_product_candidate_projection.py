...
async def results():
    ...
    output = {}
    for index, identifier in enumerate(("P1001-S2", "P1052-S2", "P1001-S1")):
...
async def test_parallel_real_tool_completions_reverse_order_keep_both_exact_skus():
    ...
    class DelayedUseCase:
        async def execute(self, spec):
            ...
            if "P1052" in spec.normalized_query:
                completed_second.set()
            return found
    events = []
    adapter = AGUIRunAdapter(request("比较 P1001-S2 / P1052-S2，供货到中国。"), events.append)
    adapter.start()
    ...
    try:
        with observe_run_events(adapter.on_trade_event):
            await asyncio.gather(tool(normalized_query="P1001-S2", ship_to="CN"),
                                 tool(normalized_query="P1052-S2", ship_to="CN"))
    finally:
        ShoppingContext.reset(token)
    snapshots = [event.snapshot for event in events if event.type == "STATE_SNAPSHOT"]
    assert [card["product_id"] for card in snapshots[-2]["products"]] == ["P1052"]
    assert [card["product_id"] for card in snapshots[-1]["products"]] == ["P1001", "P1052"]
    assert [card["default_sku_id"] for card in snapshots[-1]["products"]] == ["P1001-S2", "P1052-S2"]
...
async def test_duplicate_refs_and_source_mutation_do_not_duplicate_or_corrupt_cards(results):
    projection = ProductCandidateProjection("比较 p1001-s2 和 P1052-S2，再看 P1001-S2")
    projection.apply(results["P1052-S2"])
...
async def test_original_sku_filter_and_filtered_out_do_not_resurrect_old_candidate(results):
    projection = ProductCandidateProjection("P1001-S2 与 P1052-S2 对比")
    projection.apply(results["P1001-S1"])
    assert projection.result["hits"] == []
    projection.apply(results["P1001-S2"])
    filtered = {**results["P1052-S2"], "filtered_out": [{"product_id": "P1001", "reason": "超预算"}]}
    projection.apply(filtered)
    assert [card["product_id"] for card in projection.result["hits"]] == ["P1052"]


async def test_new_run_last_user_only_and_ordinary_query_keep_replace_semantics(results):
    first = AGUIRunAdapter(request("P1001-S2 和 P1052-S2"), lambda _: None)
    for key in ("P1001-S2", "P1052-S2"):
        first.on_trade_event(TradeEvent("s", "tool.result", results[key], "now"))
    assert len(first.state["products"]) == 2
    emitted = []
    second = AGUIRunAdapter(request("换成边缘控制器", run="run-2", state=copy.deepcopy(first.state)), emitted.append)
    second.start()
    assert second.state["products"] == []
    for key in ("P1001-S2", "P1052-S2"):
        second.on_trade_event(TradeEvent("s", "tool.result", results[key], "now"))
    assert [card["product_id"] for card in second.state["products"]] == ["P1052"]
...
def test_errors_partial_payloads_and_other_tools_do_not_fake_search_completion():
    projection = ProductCandidateProjection("P1001 和 P1052")
