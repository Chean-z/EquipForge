# EquipForge · 多智能体协同的智能装备选型与决策平台

基于 **AgentScope 2.x + FastAPI + AG-UI + React** 的智能装备选型 Agent 工程。工程师或采购人员用自然语言描述应用场景、预算、接口与性能要求，Agent 调用设备检索、选型知识和采购意向工具，页面实时渲染回答、设备卡和确认单；个人 Skill 与长期偏好可以直接在页面编写和保存。

这是带持久化、运行恢复、观测和评测机制的课程实战工程。仓库中的厂商、设备型号、技术参数、参考价格与库存均为可复现的合成演示数据；采购意向是本地业务账本，尚未接入真实询价、合同、支付、安装或售后系统。代码功能已交付，但正式检索与 Agent 质量门禁仍有 BLOCK，具体边界见[实施与验证总记录](docs/全计划实施与验证记录-2026-09-09.md)。
...
克隆并在 Codex 中打开 **`EquipForge` 目录**（包含本 README、`pyproject.toml`、`frontend/`）：
...
以下命令均从 `EquipForge` 根目录执行。Python 版本范围由 `pyproject.toml` 约束；Node.js 22 与前端 Docker 构建保持一致。模型服务须支持 OpenAI 兼容协议、工具调用和流式输出，且当前账户能访问所选模型。
...
只有 `LLM_API_KEY` 非空才允许启动；占位字符串不能用于真实对话。Embedding 默认复用 LLM 的网关和密钥，网关不支持 embedding 时需单独配置 `EMBEDDING_BASE_URL`、`EMBEDDING_API_KEY`。环境变量优先于根目录 `.env`。首次启动会加载设备目录、初始化持久库，并尝试建立设备和知识向量索引，可能需要一段时间和 embedding 调用费用。
...
健康接口应返回 `status: ok`，数据库与启用的 Redis 不应报错；该检查不保证模型、精排服务可用，还要在页面发送一次“预算 5000 元，为流水线外观缺陷检测选择一款全局快门工业相机”。检查流式回答、设备卡及运行记录。接口交互文档位于 [http://127.0.0.1:8000/docs](http://127.0.0.1:8000/docs)。
...
| 入口 | 使用方式与实际行为 |
| --- | --- |
| 设备选型 | 输入场景、参数、预算和供货地区；流式回答、设备卡、规格详情、2–3 件比较、备选收藏 |
...
| 采购意向 | Agent 或页面生成确认单，用户明确批准后才执行本地意向单与库存事务 |

例如：“记住我优先使用 GigE 接口”“把定位精度要求改为 0.1mm”“删除优先 GigE 这条偏好”。只点保存成功的条目才是已持久化内容；未保存表单不是长期记忆。正在执行的一轮使用已加载上下文，修改在下一轮生效。个人 Skill 只指导现有工具使用，不允许用户上传脚本或扩大工具权限。
...
```mermaid
flowchart TD
    UI[React 页面：对话、设备卡、Skill、偏好] --> Client[官方 AG-UI Client]
...
- **前端与传输**：React 18、TypeScript、Vite；官方 `@ag-ui/client/core 0.0.59` 对接后端 `ag-ui-protocol 0.1.22`。组件消费结构化设备和确认数据，不从模型 Markdown 猜测参数、价格、库存或意向单状态。
...
- **检索与知识**：版本化 JSONL 合成设备目录装入内存仓储；embedding → Qdrant → HTTP reranker，两级降级到向量排序或关键词。精确设备/SKU ID 可直接查目录；预算、供货地区等硬约束由代码过滤。选型知识使用 Markdown + AgentScope KnowledgeBase。BM25/RRF Hybrid 已实现，默认关闭，尚未证明收益。
...
当前目录规模为 500 SPU / 705 SKU；设备、厂商、型号、参数、价格和库存均为合成演示数据，不代表真实供给或采购建议。正式检索与 Agent 评测集将随 EquipForge 场景继续更新；精排可用性与独立 release 达标、真实登录/跨设备身份、worker 的 Langfuse 远端验收，以及 Hybrid/A-B/策略收益仍需继续验证；已完成机制不会自动晋升为生产效果结论。
